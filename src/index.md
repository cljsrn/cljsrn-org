...
### Expo
[Expo](https://expo.io/) is a tooling around react native that lets you build mobile apps using only Javascript. There is a [leiningen template](https://github.com/seantempesta/expo-cljs-template) which provides a ready-to-use setup for Clojurescript apps. The template provides REPL and figwheel capabilities using only Clojure scripts directly in user space (as opposed to re-natal external tools). This, however, also makes it more difficult to update since the code has to be patched on the user repo. The Expo template is the more recent of all approaches but it allows a to build apps without any Android or iOS SDK which is great to get started.
